...
class InverseModels(nn.Module):
    def __init__(self, state_dim, action_dim, repr_dim):
        super(InverseModels, self).__init__()

        self.trunk = nn.Linear(2*repr_dim, 2*64)
    ...
    def forward(self, embedding, embedding_next):
        return self.sample_inverse_model(embedding, embedding_next)

    def forward_inv_a(self, embedding, embedding_next):
        hh = torch.cat((embedding, embedding_next), dim=-1)
        a = self.inverse_model_action(self.trunk(hh))
        return self.lS(torch.clamp(a,-10,10))
    
    def forward_inv_reward(self, embedding, embedding_next):
        hh = torch.cat((embedding, embedding_next), dim=-1)
        r = self.inverse_model_reward(self.trunk(hh))
        return r
...
class SoftmaxCritic(nn.Module):
    def __init__(self, state_dim, action_dim, repr_dim):
        super(SoftmaxCritic, self).__init__()
        
        self.action_dim = action_dim
        self.state_dim = state_dim

        self.trunk = nn.Linear(repr_dim, 64)
    ...
    def forward(self, embedding):
        return self.value_net(self.trunk(embedding))

    def value_net(self, embedding):
        q1 = self.value_function(self.trunk(embedding))   
        return q1
    
    def critic_net(self, embedding):
        q1 = self.Q1(self.trunk(embedding))   
        q2 = self.Q2(self.trunk(embedding))
        return q1, q2
    
    def critic_target(self, embedding):
        q1 = self.critic_target_Q1(self.trunk(embedding))   
        q2 = self.critic_target_Q2(self.trunk(embedding))
        return q1, q2 


            
class SoftmaxActor(nn.Module):
    def __init__(self, state_dim, action_dim, repr_dim, use_bn=True):
    ...
    def forward(self, embedding):
        a = self.actor(self.trunk(embedding))
        return self.lS(torch.clamp(a,-10,10))
...
class Discriminator_GAN(nn.Module):
    def __init__(self, repr_dim, embedding_size = 64):
        super(Discriminator_GAN, self).__init__()
        
        self.trunk = nn.Linear(repr_dim, embedding_size)
    ...
    def forward(self, embedding):
        return self.discriminator(self.trunk(embedding))
...
class Encoder(nn.Module):
    def __init__(self, state_dim, use_bn=True):
        ...
        self.repr_dim = self.image_embedding_size
